...
def ViewProduction(request, pk):
    ...
    if pk > 0:
        ...
        for i in filter_records:
            data.append({'id': i.id, 'product':{'id': i.product.id, 'name':i.product.name, 'rate':i.product.rate}, "employee":{'id':i.employee.id, 'name':i.employee.name}, "quantity":f"{int(i.quantity) if i.quantity % 1 == 0 else i.quantity} {i.product.category.unit}", 'rate': i.product.production_cost, 'payment':i.payment,'date': i.created_at.date().strftime("%d %b %y")})
